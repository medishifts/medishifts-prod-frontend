...
export default function ApproveProfessionals() {
  ...
  const fetchData = async () => {
    try {
      const resultList = await pb
        ...
        .getList(1, 50, {
          filter: "is_email_verified=true && admin_verified=false",
        });
  ...
  const handleConfirm = async () => {
    ...
    const message =
      modalAction === "disapprove" ? { reasonForDisapproval } : "N/A";
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">
        View Approval Requests by Professionals
      </h1>
      <div className="mb-4">
        <Input
          isClearable
      ...
      <ConfirmationModal
        ...
        showReasonInput={modalAction === "disapprove"} // Show input field if disapproving
        reason={reasonForDisapproval} // The current state of the reason input
        onReasonChange={setReasonForDisapproval} // Function to update the reason input state
      />
    </div>
  );
};
